...
def style_generic_performance_dataframe(df, theme="light"):
    ...
    def color_action_perf(val):
        s_val = str(val)
        if "BUY" in s_val or "STRONG" in s_val or "CONFIRM" in s_val:
            return f"background-color:{c_green_bg};color:{c_green_txt};font-weight:700;"
        elif "WATCH" in s_val:
            return f"background-color:rgba(217, 119, 6, 0.12);color:#b45309;font-weight:600;"
        else:
            return ""
    ...
    if "Action" in df.columns:
        st_df = st_df.map(color_action_perf, subset=["Action"])
    if map_cols_outcome:
